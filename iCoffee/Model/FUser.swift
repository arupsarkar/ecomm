...
//  iCoffee
//
//  Created by Arup Sarkar on 1/23/21.
//
        ...
        Auth.auth().signIn(withEmail: email, password: password) { (authDataResult, error) in
            if error == nil {
                if authDataResult!.user.isEmailVerified {
                    //read FUser object
                    readUserFromFirestore(userId: authDataResult!.user.uid, email: email) { (error) in
...
func readUserFromFirestore(userId: String, email: String, completion: @escaping (_ error: Error? ) -> Void ) {
...
func saveUserToFirestore(fUser: FUser) {
    FirebaseReference(.User).document(fUser.id).setData(userDictionaryFrom(user: fUser)) { (error) in
        if error != nil {
            print("Error createing fUser object in firestore", error?.localizedDescription)
        }
        
    }
}

func saveUserLocally(userDictionary: NSDictionary) -> Void {
